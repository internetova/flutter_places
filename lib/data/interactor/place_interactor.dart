...
/// Так как я ранее не имела опыта работы с сетью некоторые аспекты мне до конца
/// не понятны и поэтому, вероятно, я делаю что-то не так.
//
// Вопросы дублирую тут, чтобы не забыли ответить! 🤗
//
// 1. Когда мы отправляем запрос данные должны отображаться сразу из сети, или
// их сначала надо получить, потом сохранить в память (базу данных или ещё куда)
// и оттуда уже показывать пользователю?

// 2. Когда необходимо отображать состояние иконки Избранное мы должны
// сравнивать поступающие новые данные из сети со списком Избранного на
// устройстве пользователя и иметь возможность менять это состояние, значит
// все новые данные необходимо сохранять на устройстве чтобы иметь возможность
// сделать изменение?

// 3. При переходе на детальную страницу данные передаются из уже загруженной
// карточки, или надо опять пойти в сеть и получить данные из сети?

// 4. Список Избранного - хранится у пользователя. Когда надо обновлять данные
// пользователя на свежие данные из сети? Каждый раз при заходе в Избранное?
// При переходе на детальную страницу? При новом запуске приложения?

// 5. Как часто надо обновлять distance? Вот получила я это расстояние с
// сервера, но позже оно может измениться. Его надо постоянно пересчитывать
// или в какой-то момент?
//
// 6. При get запросе мест есть параметры:
// count
// offset
// pageBy
// pageAfter
...
// sortBy
//
// Что это и к чему это? Я так понимаю, что это какая-то разбивка на страницы?
// Пока она нигде не применяется и что с этим делать не понятно.
//
// Короче, мне не совсем понятна логика как это должно работать.
// Прошу объяснить простым языком.
//
//
// Спасибо 🧐
//--------------------------------------
